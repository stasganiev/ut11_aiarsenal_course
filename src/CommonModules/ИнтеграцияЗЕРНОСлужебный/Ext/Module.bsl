...
Функция АдресРФXDTO(Адрес, Знач АдресСтрокой, СообщениеXML, ПараметрыОбмена) Экспорт
	...
	ДополнитьАдресРФXDTO_1_0_8(АдресРФXDTO, ДанныеАдреса, СообщениеXML, АдресСтрокой);
	Возврат АдресРФXDTO;
	
КонецФункции

Функция АдресРФПоКлючуАдресовXDTO(АдресСтрокой, Индекс, СообщениеXML, ПараметрыОбмена, ДанныеАдреса = Неопределено) Экспорт
...
Функция АдресПеревозкиXDTO(Адрес, АдресСтрокой, КешОшибок, ПараметрыОбмена, ТребуетсяПреобразованиеJSON = Ложь) Экспорт
	...
	Если ДанныеАдреса.КодСтраны = "643" Тогда // Россия
		Если ТребуетсяПреобразованиеJSON Тогда
			АдресРФXDTO = АдресРФПоКлючуАдресовXDTO(Адрес, АдресСтрокой, КешОшибок, ПараметрыОбмена, ДанныеАдреса);
		Иначе
...
Процедура ДополнитьАдресРФXDTO_1_0_8(АдресРФXDTO, ДанныеАдреса, СообщениеXML, АдресСтрокой)
	...
	ИначеЕсли ТипЗнч(ДанныеАдреса.ДополнительныеКоды) = Тип("Структура")
		И ДанныеАдреса.ДополнительныеКоды.Свойство("ОКТМО") Тогда
