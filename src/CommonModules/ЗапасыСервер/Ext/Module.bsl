...
Процедура ПроверитьЗаполнениеНомеровГТД(ДокументОбъект, Отказ, ПараметрыПроверки = Неопределено) Экспорт
	...
	|	ЛОЖЬ						КАК ОшибкаНекорректныйТипНомераГТД
	|ИЗ
	|	ТаблицаТовары КАК ТаблицаТовары
	...
	|	ИСТИНА						КАК ОшибкаНекорректныйТипНомераГТД
	|ИЗ
	|	ТаблицаТовары КАК ТаблицаТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|		ПО ТаблицаТовары.Номенклатура = СправочникНоменклатура.Ссылка
	...
	Если ИспользоватьУчетПрослеживаемыхИмпортныхТоваров Тогда
		ШаблонОшибкаНекорректныйТипНомераГТД = НСтр("ru = 'В строке %1 списка ""%2"" указан номер ГТД некорректного типа. Укажите номер ГТД типа ""РНПТ"" или ""РНПТ комплекта""'");
	Иначе
		ШаблонОшибкаНекорректныйТипНомераГТД = НСтр("ru = 'В строке %1 списка ""%2"" указан номер ГТД некорректного типа. Укажите номер ГТД типа ""ГТД""'");
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ОшибкаНекорректныйТипНомераГТД Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						ШаблонОшибкаНекорректныйТипНомераГТД,
						Выборка.НомерСтроки,
						ПредставлениеТЧ);
