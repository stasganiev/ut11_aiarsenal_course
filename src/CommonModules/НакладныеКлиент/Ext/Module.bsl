...
Процедура ПредупреждениеОНаличииИсправления(ИсправительнаяНакладная, ИмяФормы) Экспорт
	...
	НадписьИнфоЗаголовок = НСтр("ru = 'Накладную нельзя перезаполнить, так как для нее введена исправительная накладная:'");
	НадписьИнфоЗаголовок= НадписьИнфоЗаголовок + " " +Строка(ИсправительнаяНакладная);
